...
def get_talent_manager_jobs(pages=5):
    ...
    try:
        for page in range(1, pages + 1):
            ...
            job_cards = soup.select("div.job-listing") or soup.select("div.job") or soup.select("div.job-card")
            print(f"🧪 TM page {page}: Found {len(job_cards)} job cards")
            if not job_cards:
            ...
            for card in job_cards:
                try:
                    ...
                    company_elem = card.select_one(".company") or card.select_one(".job-company")
